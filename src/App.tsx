...
function App() {
  ...
  // Check for API key or Telegram config
  useEffect(() => {
    const checkApiKey = async () => {
      try {
        ...
        // If no key is found, show the settings dialog after a short delay
        if (!hasKey) {
          setTimeout(() => {
  ...
  // Listen for Telegram send results
  useEffect(() => {
    const unsubscribe = window.electronAPI.onTelegramSendResult((success, message) => {
      showToast(
