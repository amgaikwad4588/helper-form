...
export class ShortcutsHelper {
  ...
  public registerGlobalShortcuts(): void {
    ...
    // Take screenshot and send to Telegram shortcut
    globalShortcut.register("CommandOrControl+Z", async () => {
      console.log("Command/Ctrl + Z pressed. Taking screenshot and sending to Telegram...")
      const mainWindow = this.deps.getMainWindow()
      if (mainWindow) {
        try {
          // Get config
          ...
          if (!config.telegramEnabled || !config.telegramBotToken || !config.telegramChatId) {
            ...
            mainWindow.webContents.send("telegram-send-result", false, "Telegram not configured")
            return
          }
          
          // Increment request count
          config.telegramRequestCount = (config.telegramRequestCount || 0) + 1
          configHelper.saveConfig(config)
          const requestNumber = config.telegramRequestCount
          ...
          const caption = `📸 Request #${requestNumber}`
          const result = await telegramHelper.sendScreenshot(screenshotPath, caption)
          
          if (result.success) {
            console.log("Screenshot sent to Telegram successfully")
            mainWindow.webContents.send("telegram-send-result", true, `Screenshot sent (Request #${requestNumber})`)
          } else {
            console.error("Failed to send screenshot:", result.error)
            mainWindow.webContents.send("telegram-send-result", false, "Failed to send screenshot")
          }
          
          // Show window again
          mainWindow.setOpacity(1)
          
          // Also notify renderer about the screenshot for local queue
          const preview = await this.deps.getImagePreview(screenshotPath)
          mainWindow.webContents.send("screenshot-taken", {
            path: screenshotPath,
            preview
          ...
          console.error("Error in Ctrl+Z shortcut:", error)
          mainWindow.webContents.send("telegram-send-result", false, "Error: " + (error as Error).message)
          if (mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.setOpacity(1)
          }
        }
