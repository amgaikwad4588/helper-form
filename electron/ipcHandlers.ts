...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  ipcMain.handle("check-api-key", () => {
    // Allow app to run if either API key OR Telegram is configured
    const config = configHelper.loadConfig();
    const hasApiKey = configHelper.hasApiKey();
    const hasTelegram = config.telegramEnabled && config.telegramBotToken && config.telegramChatId;
    return hasApiKey || hasTelegram;
  })
  ...
  // Telegram handlers
  ipcMain.handle("send-screenshot-to-telegram", async (_event, imagePath: string, caption?: string) => {
    try {
      return await deps.sendScreenshotToTelegram(imagePath, caption)
    } catch (error) {
      console.error("Error sending screenshot to Telegram:", error)
      return { success: false, error: "Failed to send screenshot to Telegram" }
    }
  })

  ipcMain.handle("test-telegram-connection", async () => {
    try {
      return await deps.testTelegramConnection()
    } catch (error) {
      console.error("Error testing Telegram connection:", error)
      return { success: false, error: "Failed to test Telegram connection" }
    }
  })
}
