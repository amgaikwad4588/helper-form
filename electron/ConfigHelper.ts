...
export class ConfigHelper extends EventEmitter {
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      // Only allow latest Gemini models with best quota
      const allowedModels = ['gemini-2.5-flash', 'gemini-2.5-flash-lite', 'gemini-2.5-pro', 'gemini-3-flash', 'gemini-3.1-flash-lite', 'gemini-3.1-pro'];
      if (!allowedModels.includes(model)) {
