...
const state = {
  ...
  telegramHelper: null as TelegramHelper | null,
...
export interface IIpcHandlerDeps {
  ...
  moveWindowUp: () => void
  moveWindowDown: () => void
  sendScreenshotToTelegram: (imagePath: string, caption?: string) => Promise<{ success: boolean; error?: string }>
  testTelegramConnection: () => Promise<{ success: boolean; error?: string }>
}
...
function initializeHelpers() {
  ...
  state.telegramHelper = new TelegramHelper({
...
async function initializeApp() {
  try {
    ...
    initializeIpcHandlers({
      ...
      moveWindowUp: () => moveWindowVertical((y) => y - state.step),
      moveWindowDown: () => moveWindowVertical((y) => y + state.step),
      sendScreenshotToTelegram: async (imagePath: string, caption?: string) => {
        // Update Telegram config before sending
        const currentConfig = configHelper.loadConfig()
        state.telegramHelper?.updateConfig({
          botToken: currentConfig.telegramBotToken || "",
          chatId: currentConfig.telegramChatId || "",
          enabled: currentConfig.telegramEnabled || false
        })
        return state.telegramHelper?.sendScreenshot(imagePath, caption) || { success: false, error: "Telegram not initialized" }
      },
      testTelegramConnection: async () => {
        ...
        return state.telegramHelper?.testConnection() || { success: false, error: "Telegram not initialized" }
      }
    })
    await createWindow()
