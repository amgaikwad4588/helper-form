...
const electronAPI = {
  ...
  sendScreenshotToTelegram: (imagePath: string, caption?: string) => 
    ipcRenderer.invoke("send-screenshot-to-telegram", imagePath, caption),
  testTelegramConnection: () => ipcRenderer.invoke("test-telegram-connection"),
  onTelegramSendResult: (callback: (success: boolean, message: string) => void) => {
    const subscription = (_: any, success: boolean, message: string) => callback(success, message)
    ipcRenderer.on("telegram-send-result", subscription)
    return () => {
      ipcRenderer.removeListener("telegram-send-result", subscription)
    }
  }
}
